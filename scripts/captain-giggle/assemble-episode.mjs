...
const plan=JSON.parse(fs.readFileSync(path.join(root,'docs/captain-giggle/episode-segments.json'),'utf8'));
...
const outputDir=path.join(root,'episode-renders/captain-giggle');fs.mkdirSync(outputDir,{recursive:true});
...
fs.copyFileSync(path.join(root,'public/adventures/captain-giggle/captions.vtt'),path.join(outputDir,'captions-DRAFT-needs-final-audio-alignment.vtt'));
